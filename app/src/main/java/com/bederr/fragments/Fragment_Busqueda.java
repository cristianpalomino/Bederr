 ...
 */
public class Fragment_Busqueda extends Fragment_Master implements View.OnFocusChangeListener, Operation_Distritos.Interface_Operation_Distritos {
        ...
        action_right.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (edtcategorias.getText().toString().matches("") && edtdistritos.getText().toString().matches("")) {
                    ...
                    getActivity().getSupportFragmentManager().beginTransaction().setCustomAnimations(R.animator.izquierda_derecha_b, R.animator.izquierda_derecha_b).add(R.id.container, Fragment_Resultado_Busquedas.newInstance(s, distrito),Fragment_Resultado_Busquedas.class.getName()).addToBackStack(null).commit();
                }
                hideSoftKeyboard();
            }
        });
